...
def test_event_3_attendees(django_user_model):
    ...
    Attendance.objects.create(event=event, user=user_1)
    Attendance.objects.create(event=event, user=user_2)
    Attendance.objects.create(event=event, user=user_3)
    c = Client()
    response = c.get("/" + cult.slug + "/" + event.slug + "/")
    assert response.status_code == 200
    assert cult.name.encode() in response.content
...
def test_event_form(django_user_model):
    ...
    Membership.objects.create(cult=cult, user=user, role=Membership.LEADER)
    form_data = {
        "title": "Stayin alive",
...
def test_event_form_member(django_user_model):
    ...
    Membership.objects.create(cult=cult, user=user, role=Membership.MEMBER)
    form_data = {
        "title": "Stayin alive",
        "details": "Whether you are a brother or whether you are a mother",
        "date": "2018-02-02",
...
def test_event_post_edit_form_leader(django_user_model):
    ...
    assert logged_in
    response = c.post(
        "/" + cult.slug + "/" + event.slug + "/edit/", form_data, follow=True
    )
    assert response.status_code == 200
