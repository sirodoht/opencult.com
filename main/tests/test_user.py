...
def test_profile(django_user_model):
    ...
    Membership.objects.create(cult=cult_1, user=user, role=Membership.LEADER)
    Membership.objects.create(cult=cult_2, user=user, role=Membership.MEMBER)
    c = Client()
...
def test_email_form(django_user_model):
    ...
    user.set_password("takeajacket")
    user.save()
    form_data = {"email": "daddy@cool.com"}
    ...
    assert response.status_code == 200
    assert (
        b"Login email sent! Please check your inbox and click on the link."
        in response.content
    )
...
def test_edit_user(django_user_model):
    ...
    user.save()
    form_data = {"username": "daddycool", "about": "Watching Grease"}
